...
export default function DisponibilidadScreen({ route }: any) {
  ...
  // Generar 14 días a partir de hoy
  ...
  return (
    <SafeAreaView style={styles.container}>
        ...
        {horariosDisponibles.length > 0 ? (
          horariosDisponibles.map((hora) => {
            ...
            return (
              <TouchableOpacity
                key={hora}
                style={[styles.horaCard, { width: horarioCardWidth }, isSelected && styles.selectedHoraCard]}
                onPress={() => setSelectedHora(hora)}
              >
                <Text style={[styles.horaText, isSelected && styles.selectedHoraText]}>{hora}</Text>
              </TouchableOpacity>
            );
          })
        ) : (
      ...
      >
        <Text style={styles.reservarText}>{reservaLoading ? "Reservando..." : "Reservar"}</Text>
      </TouchableOpacity>
    </SafeAreaView>
  );
}
